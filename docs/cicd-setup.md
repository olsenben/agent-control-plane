...
CT103: app host only, runtime .env on disk, no act_runner
```
...
Only deployment secrets — see [gitea-secrets.md](gitea-secrets.md):

- `DEPLOY_SSH_KEY`
- `DEPLOY_HOST` (`192.168.4.62`)
- `DEPLOY_USER` (`deploy`)

Runtime secrets live in CT103 `.env` — see [secrets-boundaries.md](secrets-boundaries.md).
...
### deploy user SSH (CT102 → CT103)

1. Generate keypair off-host: `ssh-keygen -t ed25519 -f ./ct103_deploy`
2. Private key → Gitea `DEPLOY_SSH_KEY`
3. Public key → `/home/deploy/.ssh/authorized_keys` on CT103
...
merge to main → deploy on deploy (test → SSH deploy → readyz gate)
```

Trigger manual deploy: Actions → deploy → Run workflow.
