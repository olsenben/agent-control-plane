#!/usr/bin/env bash
# CT103 smoke for V7 T05 bake-off report (deploy agents run after tip pin).
set -euo pipefail
...
cat > /tmp/v7_t05_smoke.py <<'PY'
from pathlib import Path
import tempfile
from agent_control.bakeoff_report import emit_bakeoff_report_for_bundle
...
run_id = "run-v7t05-smoke"
session = AgentSession(
    session_id="sess-v7t05-smoke",
    project=project,
    repo="demo-app",
    subject_kind="issue",
    subject_number=1,
...
append_control_decision(root, project=project, kind="other", summary="t05", session_id=session.session_id, run_id=run_id, trace_id=session.trace_id)
_, bpath = export_eval_bundle(root, project=project, run_id=run_id, output_dir=root/"exp")
report, path, results = emit_bakeoff_report_for_bundle(bpath, output_dir=root/"report")
assert path.is_file()
assert report["schema_version"] == "bakeoff_report.v1"
...
print("V7_T05_SMOKE_OK profiles", len(results), "gates_ok", report["production_gates"]["all_passed"])
PY
CID=\$(docker compose ps -q control-plane)
docker cp /tmp/v7_t05_smoke.py "\$CID":/tmp/v7_t05_smoke.py
docker compose exec -T control-plane python /tmp/v7_t05_smoke.py </dev/null
EOS
...
echo DEPLOY_SMOKE_V7_T05_PASS tip=$TIP
