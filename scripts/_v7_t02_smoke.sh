...
run_id = "run-v7t02-smoke"
session = AgentSession(
    session_id="sess-v7t02-smoke",
    project=project,
    repo="demo-app",
    subject_kind="issue",
    subject_number=1,
...
assert {d["profile_id"] for d, _ in results} == set(PROFILE_IDS)
shas = {d["source_eval_bundle_sha256"] for d, _ in results}
assert len(shas) == 1
ns = {d["memory_namespace"] for d, _ in results}
assert len(ns) == 4
assert all(d["production_memory_touched"] is False for d, _ in results)
print("V7_T02_SMOKE_OK", "profiles", len(results), "sha", next(iter(shas))[:12])
PY
CID=\$(docker compose ps -q control-plane)
docker cp /tmp/v7_t02_smoke.py "\$CID":/tmp/v7_t02_smoke.py
docker compose exec -T control-plane python /tmp/v7_t02_smoke.py </dev/null
EOS
...
echo DEPLOY_SMOKE_V7_T02_PASS tip=$TIP
